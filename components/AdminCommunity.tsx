    ...
    <div className="space-y-6 py-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        {communityStats.map(s => (
          <div key={s.title} className="glass-card p-3 sm:p-4 rounded-xl border border-purple-500/12 flex flex-col justify-between space-y-2">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-purple-300 bg-purple-500/10 border border-purple-500/20 px-2.5 py-1 rounded-lg self-start">
            {highlightedStudents.length} aluno(s) em destaque
          </span>
        </div>

        {students.length === 0 ? (
          <div className="p-6 text-center glass-card rounded-xl border border-purple-500/5">
            <Users className="w-6 h-6 text-purple-500/50 mx-auto mb-2" />
                ...
                <div 
                  ...
                  className={`glass-card p-3 sm:p-4 rounded-xl border flex items-center justify-between gap-3 transition-all ${
                    isHighlighted 
                      ...
                      : 'border-purple-500/10 bg-[#0a0715]/40'
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-9 h-9 rounded-full overflow-hidden border flex-shrink-0 ${
                      isHighlighted ? 'border-yellow-500/40 ring-2 ring-yellow-500/20' : 'border-purple-500/20'
                    } bg-purple-950/30`}>
                      ...
                      {!student.isHighlightedNetworking && (
                        <div className="w-full h-full bg-purple-950/40 flex items-center justify-center text-gray-400 font-bold text-xs">
                          {student.name.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()}
                        </div>
                      )}
                    </div>
      ...
      <div className="space-y-3.5">
        ...
        {posts.length === 0 ? (
          <div className="p-6 text-center glass-card rounded-xl border border-purple-500/5">
            <MessageSquare className="w-6 h-6 text-purple-500/50 mx-auto mb-2" />
            ...
            <AnimatePresence mode="popLayout">
              {posts.map(post => (
                <motion.div
                  ...
                  className="glass-card p-3 sm:p-4 rounded-xl border border-purple-500/10 flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4 bg-[#0a0715]/40"
                >
