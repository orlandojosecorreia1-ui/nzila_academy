...
export default function AdminFinancials() {
  ...
  const distributions = [
    { method: 'Cartão de Crédito', pct: 65, count: 124, color: 'bg-purple-500', text: 'text-purple-400' },
    { method: 'Pix Bancário', pct: 28, count: 53, color: 'bg-cyan-500', text: 'text-cyan-400' },
    { method: 'Boleto Bancário', pct: 7, count: 14, color: 'bg-indigo-500', text: 'text-indigo-400' }
  ];
  ...
  return (
    <div className="space-y-6 py-2">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-purple-950/20 pb-4">
        <div>
          <h2 className="text-xl font-display font-medium text-white flex items-center gap-2">
            <Landmark className="w-5 h-5 text-purple-400" /> Painel Geral Financeiro
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Total Faturamento */}
        <div className="glass-card p-5 rounded-xl border border-purple-500/15 flex items-center justify-between">
          <div className="space-y-1.5">
            ...
            <span className="text-[10px] text-purple-400 font-mono block">Volume transicionado do sandbox</span>
          </div>
        ...
        {/* Métodos preferidos */}
        <div className="glass-card p-5 rounded-xl border border-purple-500/15 flex items-center justify-between col-span-2 bg-[#100c25]/30">
          <div className="w-full space-y-3">
            <span className="text-[10px] text-purple-300 font-mono font-bold tracking-wider block">PREFERÊNCIA DE PAGAMENTOS (MÉDIAS DE MERCADO)</span>
            ...
            <div className="grid grid-cols-3 gap-4 text-xs">
              {distributions.map(d => (
                <div key={d.method} className="space-y-1 font-mono">
                  ...
                  <div className="w-full bg-purple-950/30 h-1.5 rounded-full overflow-hidden">
      ...
      <div className="glass-card rounded-2xl overflow-hidden border border-purple-500/12">
        <div className="p-4 bg-purple-950/15 border-b border-purple-950/30">
          <h3 className="text-xs font-mono font-bold tracking-widest text-[#a78bfa] uppercase">HISTÓRICO RECENTE DE TRANSAÇÕES</h3>
        </div>
        
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-gray-300 font-mono">
            <thead className="bg-[#0b0813] text-gray-400 text-[10px] font-bold border-b border-purple-950/30 uppercase">
              <tr>
            ...
            <tbody className="divide-y divide-purple-950/15">
              {transactions.map((t, i) => (
                <tr key={t.id} className="hover:bg-purple-950/5 transition-all">
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center gap-1">
                      <CreditCard className="w-3.5 h-3.5 text-purple-400" /> {t.paymentMethod}
                    </span>
                  </td>
