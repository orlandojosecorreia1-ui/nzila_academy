    ...
    <div className="space-y-4 sm:space-y-6 py-2 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-4 sm:space-y-6 order-2 lg:order-1">
          ...
          <form onSubmit={handleCreatePost} className="glass-card p-3 sm:p-4 rounded-xl border border-purple-500/10 space-y-3 sm:space-y-3.5 bg-[#0e0a1a]/40">
            <div className="flex gap-2 sm:gap-3">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden flex-shrink-0 border border-purple-500/20 bg-purple-950/20">
                <img 
              ...
              <div className="flex-grow">
                <textarea
                  ...
                  className="w-full bg-black/20 rounded-lg p-2.5 sm:p-3 text-xs sm:text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/10 min-h-[70px] sm:min-h-[90px] resize-none"
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1 scrollbar-none no-scrollbar -mx-1 px-1">
            <div className="text-[10px] sm:text-xs font-mono text-gray-400 flex items-center gap-1 flex-shrink-0 mr-1 bg-purple-950/10 p-1 sm:p-1.5 rounded-md border border-purple-500/5">
            ...
            {categories.map(cat => (
              <button
                ...
                className={`text-[10px] sm:text-[11px] font-mono px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full border transition-all hover:cursor-pointer flex-shrink-0 ${selectedTag === cat ? 'bg-purple-600 border-purple-400 text-white' : 'bg-[#0f0b20] border-purple-900/30 text-gray-400 hover:text-white'}`}
              >
                {cat}
              </button>
            ))}
          ...
          <div className="space-y-3 sm:space-y-4">
            <AnimatePresence mode="popLayout">
              {filteredPosts.length === 0 ? (
                <div className="p-6 sm:p-8 text-center glass-card rounded-xl border border-purple-500/5 space-y-2">
                ...
                filteredPosts.map((post) => (
                  <motion.div
                    ...
                    className={`glass-card p-3.5 sm:p-5 rounded-xl border relative transition-all bg-[#0e0a1a]/30 ${post.isPinned ? 'border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.05)] bg-gradient-to-l from-purple-950/5 to-transparent' : 'border-purple-500/10'}`}
                  >
                    {/* Pin Status */}
                    {post.isPinned && (
                      <div className="absolute top-3 sm:top-4 right-3 sm:right-4 text-[9px] sm:text-[10px] font-mono text-purple-400 flex items-center gap-1 font-bold">
                        <Pin className="w-3 sm:w-3.5 h-3 sm:h-3.5 fill-purple-400/20" /> FIXADO
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      <div 
                        ...
                        className="w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden border border-purple-950/30 flex-shrink-0 bg-purple-950/10 cursor-pointer hover:border-purple-400 transition-all"
                      >
                        <img 
                      ...
                      <div className="min-w-0">
                        <div 
                          onClick={() => handleQuickProfileView(post)}
                          className="text-xs sm:text-sm font-medium text-white hover:text-purple-300 cursor-pointer transition-colors font-display truncate"
                        >
                          {post.authorName}
                        </div>
                    ...
                    {/* Content */}
                    <div className="text-xs sm:text-sm text-gray-300 leading-relaxed pt-2 sm:pt-3 py-2 whitespace-pre-line selection:bg-purple-900/30">
                      {post.content}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1 sm:gap-1.5 pt-1 sm:pt-2">
                      {post.tags.map(tag => (
                        <span key={tag} className="text-[9px] sm:text-[10px] font-mono bg-purple-950/20 border border-purple-500/10 px-1.5 sm:px-2 py-0.5 rounded text-purple-400 font-medium">
                          #{tag}
                        </span>
                      ))}
                    </div>
                    ...
                    {/* Interaction toolbar */}
                    <div className="flex items-center gap-3 sm:gap-5 text-[10px] sm:text-xs text-gray-400 font-mono pt-3 sm:pt-4 mt-3 sm:mt-4 border-t border-purple-950/20">
                      <button 
                        onClick={() => likePost(post.id)}
                        className={`flex items-center gap-1 sm:gap-1.5 hover:text-purple-400 transition-colors hover:cursor-pointer ${post.likedByCurrentUser ? 'text-purple-400 font-semibold' : ''}`}
                        id={`feed-btn-like-${post.id}`}
                      >
                        <Heart className={`w-3.5 sm:w-4 h-3.5 sm:h-4 ${post.likedByCurrentUser ? 'fill-purple-400 text-purple-400' : ''}`} />
                    ...
                    <AnimatePresence>
                      {activeCommentsPostId === post.id && (
                        <motion.div 
                          ...
                          className="overflow-hidden bg-[#07050d] rounded-lg mt-3 sm:mt-3.5 border border-purple-950/35 p-2.5 sm:p-3.5 space-y-2.5 sm:space-y-3"
                        >
                          {/* List existing comments */}
                          {post.comments.length > 0 && (
                            <div className="space-y-2 sm:space-y-2.5 max-h-40 overflow-y-auto pr-1">
                              {post.comments.map(c => (
                                <div key={c.id} className="text-xs p-2 sm:p-2.5 rounded bg-[#0d0a1b]/80 border border-purple-950/10 space-y-1">
                                  <div className="flex justify-between items-center text-[9px] sm:text-[10px] font-mono">
                                    <span className="text-purple-300 font-medium truncate">{c.authorName}</span>
                          ...
                          <div className="flex gap-2">
                            <input
                              ...
                              className="bg-black/40 border border-purple-900/30 rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/10 flex-grow min-w-0"
                              id={`input-post-comment-${post.id}`}
                            />
                            <button
                              onClick={() => handlePostComment(post.id)}
                              className="px-2.5 sm:px-3 py-1.5 sm:py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-xs font-mono font-medium hover:cursor-pointer transition-colors flex-shrink-0"
        ...
        <div className="lg:col-span-4 space-y-4 order-1 lg:order-2">
          ...
          <div className="glass-card p-3 sm:p-4 rounded-xl border border-purple-500/10 space-y-3 sm:space-y-4">
            ...
            {highlightedStudents.length === 0 ? (
              <div className="py-4 text-center space-y-1.5">
                <Users className="w-5 h-5 text-purple-500/40 mx-auto" />
                  ...
                  <div key={student.id} className="py-2.5 sm:py-3 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="block text-[9px] sm:text-[10px] text-purple-400 font-mono truncate">{student.courseTitle || 'Aluno Nzila'}</span>
                      </div>
                    </div>
        ...
        {selectedProfile && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 z-50">
            <motion.div 
              ...
              className="w-full max-w-sm glass-card p-4 sm:p-6 rounded-2xl relative border border-purple-500/20 text-center"
            >
              <button 
              ...
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-2 border-purple-500/40 mx-auto mt-2 bg-gradient-to-br from-purple-500/20 to-cyan-500/20 flex items-center justify-center">
              ...
              {selectedProfile.wa && (
                <div className="mt-4 sm:mt-6 border-t border-purple-950/30 pt-3 sm:pt-4">
                  <a 
